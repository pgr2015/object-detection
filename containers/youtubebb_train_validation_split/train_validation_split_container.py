        ...
        with h5py.File(tensor_file) as merged_tensors:
            ...
            with h5py.File(output_file, "w") as splitted_tensors:
                ...
                def copy_values(dest_group, dest_name, indexes, input_dataset):
                    ...
                    dest_dset = dest_group.create_dataset(dest_name, new_shape, dtype=input_dataset.dtype)
                    dest_dset[:] = input_dataset[indexes]
                
                
                input_dataset = merged_tensors[INPUT_DATA_SET_NAME]
                new_shape = list(input_dataset.shape)
                new_shape[0] = len(validation_indexes)
                dest_val_in = dest_val.create_dataset(INPUT_DATA_SET_NAME, new_shape, dtype=input_dataset.dtype)
                cont = 0;
                for i in validation_indexes:
                    dest_val_in[cont] = input_dataset[i]
                    cont = cont+1
                
                input_dataset = merged_tensors[OUTPUT_DATA_SET_NAME]
                new_shape = list(input_dataset.shape)
                new_shape[0] = len(validation_indexes)
                dest_val_out = dest_val.create_dataset(OUTPUT_DATA_SET_NAME, new_shape, dtype=input_dataset.dtype)
                cont = 0;
                for i in validation_indexes:
                    dest_val_out[cont] = input_dataset[i]
                    cont = cont+1
                        
                input_dataset = merged_tensors[INPUT_DATA_SET_NAME]
                new_shape = list(input_dataset.shape)
                new_shape[0] = len(learning_indexes)
                dest_learn_in = dest_learn.create_dataset(INPUT_DATA_SET_NAME, new_shape, dtype=input_dataset.dtype)
                cont = 0;
                for i in learning_indexes:
                    dest_learn_in[cont] = input_dataset[i]
                    cont = cont+1
                      
                input_dataset = merged_tensors[OUTPUT_DATA_SET_NAME]  
                new_shape = list(input_dataset.shape)
                new_shape[0] = len(learning_indexes)
                dest_learn_out = dest_learn.create_dataset(OUTPUT_DATA_SET_NAME, new_shape, dtype=input_dataset.dtype)
                cont = 0;
                for i in learning_indexes:
                    dest_learn_out[cont] = input_dataset[i]
                    cont = cont+1
