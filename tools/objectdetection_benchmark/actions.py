...
from bonseyes_youtubebb.mobilenetSSD import proto_generator, solver_generator_test
...
def perform_bebenchmarking(context: Context[BlobDataEditor], model: BlobDataEditor, training_set: DataTensorsViewer, epochs: str, batch_size: str):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        proto_generator(train_path, "train", lmdb_path_tra, labelmap_path, 24, int(batch_size), 22)

        # ================Test.prototxt================

        test_path = os.path.join(tmp_dir, "MobileNetSSD_test.prototxt")
        proto_generator(test_path, "test", lmdb_path_test, labelmap_path, 24, int(batch_size), 22)
...
def create(context: Context[BlobDataEditor], model: BlobDataEditor, training_set: DataTensorsViewer,
           epochs: str = "5000", batch_size: str = "6"):  # tensor: DataTensorsViewer, model: BlobDataViewer, pairs: str):
