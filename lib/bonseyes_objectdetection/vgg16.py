...
"""%(self.lmdb,self.batch,self.input_size))
    ...
    def conv(self, name, top, out, kernel, pad, bottom=None):
        bias_lr_mult = ""
        weight_lr_mult = ""
        bias_filler = ""
        weight_filler = ""
...
""" % (name, bottom, top, weight_lr_mult, bias_lr_mult, out, pad, kernel, weight_filler,  bias_filler))
...
"""%(bottom, top, name)
    ...
    def generate(self, stage, lmdb,  class_num, batch, size, quantize=False):
        ...
        if stage == "train":
            ...
            self.data_deploy()
        self.conv(bottom="data", name="conv1_1", top="conv1_1", out=64, pad=1, kernel=3)
